...
class easeePlatform {
	...
	async getDevices() {
		try {
			...
			this.observations.items = await this.easeeapi.getObservations().catch(err => {
				this.log.error('Failed to get observation list for build. \n%s', err)
			})
			this.log.debug('Retrieved %s observations', observations.items.length)
			// login to the API and get the token
			let login = await this.easeeapi.login(this.username, this.password).catch(err => {
				this.log.error('Failed to get login for build. \n%s', err)
			})
			...
			//get profile
			let profile = await this.easeeapi.profile(this.token).catch(err => {
				this.log.error('Failed to get profile for build. \n%s', err)
			})
			this.log.info('Found account for %s %s', profile.firstName, profile.lastName)
			this.userId = profile.userId
			//get product
			let products = await this.easeeapi.products(this.token, this.userId).catch(err => {
				this.log.error('Failed to get products for build. \n%s', err)
			})
			products
				.filter(location => {
				...
				.forEach(async location => {
					...
					location.circuits.forEach(circuit => {
						...
						circuit.chargers.forEach(async charger => {
							...
							let chargerConfig = await this.easeeapi.chargerConfig(this.token, charger.id).catch(err => {
								this.log.error('Failed to get charger config info for build. \n%s', err)
							})
							let chargerState = await this.easeeapi.chargerState(this.token, charger.id).catch(err => {
								this.log.error('Failed to get charger state for build. \n%s', err)
							})
							let chargerDetails = await this.easeeapi.chargerDetails(this.token, charger.id).catch(err => {
								this.log.error('Failed to get charger detail info for build. \n%s', err)
							})
					...
					try {
						if (this.showEqualizer && location.equalizers[0]) {
							...
							let equalizerConfig = await this.easeeapi.equalizerConfig(this.token, location.equalizers[0].id).catch(err => {
								this.log.error('Failed to get equalizer config info for build. \n%s', err)
							})
							let equalizerState = await this.easeeapi.equalizerState(this.token, location.equalizers[0].id).catch(err => {
								this.log.error('Failed to get equalizer state for build. \n%s', err)
							})
							let equalizerDetails = await this.easeeapi.equalizerDetails(this.token, location.equalizers[0].id).catch(err => {
								this.log.error('Failed to get equalizer detail info for build. \n%s', err)
							})
						...
						this.log.warn('Failed to add Equalizer')
					}
				})
			setTimeout(() => {
				this.log.info('Easee Platform finished loading')
			}, 5000)
		} catch (err) {
	...
	setTokenRefresh(ttl) {
		setInterval(async () => {
			try {
				let tokenInfo = await this.easeeapi.refreshToken(this.token, this.refreshToken).catch(err => {
					this.log.error('Failed signin to refresh token. \n%s', err)
				})
	...
	resetSignalR(ttl, charger) {
		setInterval(() => {
			try {
				...
				this.log.error('Failed to reset SignalR. \n%s', err)
			}
		}, (ttl * 1000) / 1.2)
	}
	...
	async calcBattery(chargerId, batteryService, sensorService) {
		try {
			...
			let currentSession = await this.easeeapi.currentSession(this.token, chargerId).catch(err => {
				this.log.error('Failed to get current session. \n%s', err)
			})
	...
	async updateEq(windowService, eqId) {
		try {
			let equalizerConfig = await this.easeeapi.equalizerConfig(this.token, eqId).catch(err => {
				this.log.error('Failed to get comfig info for build. \n%s', err)
			})
			if (equalizerConfig) {
			...
			this.log.error('Failed to update equalizer. \n%s', err)
		}
	}
	...
	updateService(message) {
		...
		switch (message.dataType) {
			...
			case 2: {
				if (parseInt(message.value) == 1 || message.value == 'True') {
					value = true
				} else {
					value = false
				}
		...
		switch (message.id) {
			...
			case 109: //state_chargerOpMode
				...
				if (this.showControls == 1 || this.showControls == 2 || this.showControls == 3) {
					activeService = lockAccessory.getServiceById(Service.Switch, message.mid)
				}
				if (this.showControls == 10 || (this.showControls == 20) | (this.showControls == 30)) {
					activeService = lockAccessory.getServiceById(Service.Outlet, message.mid)
				}
				if (this.showControls == 4) {
					activeService = lockAccessory.getServiceById(Service.Thermostat, message.mid)
				}
				...
				switch (value) {
					...
					case 3: //charging
						...
						lockService.getCharacteristic(Characteristic.OutletInUse).updateValue(true)
						if (this.showBattery || this.showSensor) {
							this.calcBattery(message.mid, batteryService, sensorService)
						}
						if (this.showControls == 1 || this.showControls == 2 || this.showControls == 3) {
							activeService.getCharacteristic(Characteristic.On).updateValue(true)
						}
